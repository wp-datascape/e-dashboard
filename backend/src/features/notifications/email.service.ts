...
/** Kop statis (task016 §33) — SEKALI di awal email (bukan per-batch): judul,
 * penjelasan metodologi, cakupan periode, kriteria filter, jadwal kirim
 * otomatis. Sesuai draft user — target pembaca eksekutif yang perlu paham
 * konteks laporan SEBELUM lihat detail per jenis laporan di bawahnya. */
function buildDocumentIntroHtml(dict: ReturnType<typeof getDict>): string {
  const coverageItems = dict.document.coverageList.map(item => `<li>${escapeHtml(item)}</li>`).join('')
  const scheduleItems = dict.document.scheduleList.map(item => `<li>${escapeHtml(item)}</li>`).join('')
  return `
      <h2 style="margin:0 0 10px;color:#111827;font-size:18px;">${escapeHtml(dict.document.title)}</h2>
      <p style="margin:0 0 10px;color:#374151;font-size:14px;line-height:1.6;">${escapeHtml(dict.document.intro)}</p>
      <p style="margin:0 0 4px;color:#374151;font-size:14px;line-height:1.6;">${escapeHtml(dict.document.coverageLead)}</p>
      <ul style="margin:0 0 10px;padding-left:20px;color:#374151;font-size:14px;line-height:1.6;">${coverageItems}</ul>
      <p style="margin:0 0 14px;color:#374151;font-size:14px;line-height:1.6;">${escapeHtml(dict.document.criteriaNote)}</p>
      <div style="font-weight:700;color:#111827;font-size:14px;margin-bottom:4px;">${escapeHtml(dict.document.scheduleTitle)}</div>
      <ul style="margin:0 0 14px;padding-left:20px;color:#374151;font-size:14px;line-height:1.6;">${scheduleItems}</ul>
      <p style="margin:0 0 16px;color:#6B7280;font-size:12px;font-style:italic;line-height:1.5;">${escapeHtml(dict.document.closingNote)}</p>
      <hr style="border:none;border-top:1px solid #E5E7EB;margin:0 0 4px;"/>`
}

/** Blok metadata berlabel per jenis laporan yang masuk digest ini (biasanya
 * cuma 1, tapi 1 Januari bisa >1 saat bulanan+kuartal+semester+tahunan tutup
 * bersamaan) — Generated by/Analysis/Generated At/Report Type/Period/
 * Comparison, gaya kop dokumen formal (task016 §33, sesuai draft user). */
function buildBatchBlock(batch: ReturnType<typeof groupDigestBatches>[number], dict: ReturnType<typeof getDict>, generatedAt: string): string {
  const row = (label: string, value: string, bold = false): string =>
    `<tr><td style="padding:2px 12px 2px 0;color:#6B7280;white-space:nowrap;">${label}</td><td style="padding:2px 0;${bold ? 'font-weight:700;' : ''}">${value}</td></tr>`
  return `
      <div style="border-left:3px solid ${BRAND_COLOR};background:#F8FAFC;padding:14px 18px;margin:16px 0;">
        <table style="font-size:13px;color:#374151;border-collapse:collapse;">
          ${row(METADATA_LABELS.generatedBy, escapeHtml(dict.pdf.brandName))}
          ${row(METADATA_LABELS.analysis, METADATA_LABELS.analysisValue)}
          ${row(METADATA_LABELS.generatedAt, `${escapeHtml(generatedAt)} WIB`)}
          ${row(METADATA_LABELS.reportType, reportTypeLabel(batch.periodType, batch.checkpoint), true)}
          ${row(METADATA_LABELS.period, escapeHtml(formatDateRange(batch.current.start, batch.current.end, dict.monthNames)))}
          ${row(METADATA_LABELS.comparison, escapeHtml(formatDateRange(batch.comparison.start, batch.comparison.end, dict.monthNames)))}
        </table>
...
function buildDigestHtml(items: DigestNotificationItem[], appBaseUrl: string | null, locale: Locale): string {
  ...
  const batchBlocks = groupDigestBatches(items).map(batch => buildBatchBlock(batch, dict, generatedAt)).join('')
