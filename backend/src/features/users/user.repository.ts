...
async function fetchAssignmentTreeByUserIds(userIds: number[]) {
  ...
  const [companyRows, branchRows, divisionRows] = await Promise.all([
    ...
    db
      .select({ userId: userDivisions.user_id, branchId: userDivisions.branch_id, divisionId: userDivisions.division_id })
      .from(userDivisions)
      .where(inArray(userDivisions.user_id, userIds)),
  ])

  const divisionsByUserBranch = new Map<string, number[]>()
  for (const { userId, branchId, divisionId } of divisionRows) {
    const key = `${userId}:${branchId}`
    if (!divisionsByUserBranch.has(key)) divisionsByUserBranch.set(key, [])
    divisionsByUserBranch.get(key)!.push(divisionId)
  }
    ...
    await db.transaction(async (tx) => {
      ...
      const divisionRows = assignments.flatMap((a) =>
        a.branches.flatMap((b) =>
          b.divisions.map((divisionId) => ({ user_id: userId, branch_id: b.branch_id, division_id: divisionId })),
        ),
      )
