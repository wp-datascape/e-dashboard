          ...
          <Stack spacing={2.5}>
            ...
            {detail.items.map((item: InvoiceItem, i: number) => (
              <Box key={item.id} sx={{ p: 1.5, border: 1, borderColor: 'divider' }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <Typography variant="caption" color="text.secondary">Revenue: {formatIDR(item.revenue)}</Typography>
                  <Typography variant="caption" color={theme.palette.success.main}>GP: {formatIDR(item.gross_profit)}</Typography>
                </Box>
              </Box>
            ))}
          </Stack>
