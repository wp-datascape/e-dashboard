...
| Palette tambahan (sesi 39, 2026-07-08) | **Purple**, **Rose**, **Indigo** — total 6 | User minta lebih banyak varian. Warna dipilih menghindari tabrakan dengan warna semantik (success/warning/error/info) dan secondary palette lain yang sudah ada — mis. Indigo pakai secondary **sky** bukan cyan (cyan = warna semantik `info`, akan ambigu). **Wajib sinkron 2 tempat kalau nambah palette baru**: `frontend/src/theme/palettes.ts` (sumber utama) DAN `backend/src/features/auth/auth.schema.ts` (`COLOR_PALETTES` — validasi request PATCH, akan reject kalau tidak disinkronkan). |
