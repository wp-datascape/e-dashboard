...
| Layer    | Status | Notes                          |
|----------|--------|--------------------------------|
| Frontend | ~99%   | Button-level CRUD guards (useCan hook) di semua halaman. Filter bar (entitas+divisi+periode) di semua halaman metrics. Sidebar collapsed submenu flyout fix (sesi 26). Fix logout tidak invalidasi sesi server (sesi 29). PWA installable (service worker + icon, sesi 31), fix status bar iOS + tabel tablet (sesi 33). Semua dialog konsisten pakai komponen `Dialog` bersama — 6 raw MUI Dialog + 4 drawer detail dimigrasikan (sesi 34). Error API di-i18n via kode (`getApiErrorMessage`), 9 i18n key hilang ditambahkan, audit log action i18n dibangun ulang (13→28 action), fix bug logout redirect 404 (sesi 36). **Logo/favicon/PWA icon baru (four-leaf clover), fix bug scope-leak dropdown Company/Branch/Division (`companyId==='all'` — komponen baru `ScopeFilterFields` dipasang di 9 halaman), fix icon date/month picker invisible di light mode (7 halaman → komponen `DatePicker`), preferensi user (theme/palette/bahasa) tersimpan ke akun + avatar menu pengganti tombol Logout, AppBar ikut palette (sesi 38, Task003)**. **Fix responsive mobile (`ScopeFilterFields` full-width, `Box`+`gap` bukan `Stack`+`spacing`, word-break `ResponsiveListView`), variant Typography baru `pageTitle`/`pageSubtitle` ikut palette (22 halaman + Dialog), 3 palette baru Purple/Rose/Indigo (total 6), logo jadi outline putih transparan, fix `ThemeToggle` beda tampilan mobile-desktop (`flexShrink:0`), stagger debounce chart (sesi 39)**. **Klik baris kategori di tab "Penetrasi Kategori" (High Margin) buka dialog drill-down daftar produk (task007), lalu dibetulkan supaya cuma tampilkan produk yang BENAR-BENAR ditandai high margin — bukan semua produk kategori — plus badge "High Margin" per produk & kartu summary drill-down disinkronkan ke agregat produk terfilter (task008, 2026-07-26)**. **Fix baris tabel "naik-turun tidak sejajar" di seluruh aplikasi — `.MuiDataGrid-cell` (`ResponsiveListView`, dipakai 23 halaman) dipaksa `flex`+`align-items:center`; kolom berisi list chip (tab Target Upsell) dapat `getRowHeight="auto"` supaya semua kategori rekomendasi kelihatan penuh, tidak kepotong (2026-07-26)**. **Fix icon PWA blank/putih polos — `icon-192`, `icon-512`, `apple-touch-icon` cuma logo tanpa background gelap; `icon-maskable-512` desainnya beda sendiri (flower solid, bukan semanggi outline) — ketiganya digenerate ulang pakai desain identik `favicon.svg`/`AppLogo.tsx` (2026-07-26)**. |
| Backend  | ~98%   | Auth selesai. requirePermission di semua route. M1–M2, M8–M10, Product Trend (avg-category), Transactions, Dashboard live (real backend). API Docs (Swagger UI) 83 operasi/63 path (sesi 29). Users bulk import + reset password (sesi 30). Backend di-Dockerize + obfuscate untuk deploy Railway (sesi 31). **Fix bug RBAC: `metrics.route.ts` pakai permission deprecated (semua role non-superadmin selalu 403), `GET /companies` & High Margin List kini pakai `resolveCompanyScope` (sesi 32/34)**. Seed baseline permission otomatis utk role `admin`/`user` (sesi 32). **Isolasi data superadmin — List User & Audit Log disembunyikan total dari viewer non-superadmin (sesi 37)**. **Fix query dashboard timeout `company_id=all` (index invoices + `statement_timeout`), fix unique constraint dedup invoice yang tidak pernah ke-generate, `branch_division_enforcement_enabled` default diubah jadi `true`, endpoint self-service baru `PATCH /auth/me/preferences` (sesi 38, Task003)**. **Activity Log (auto-log semua request API + page-view eksplisit) & Login Log (login/logout/lockout/ganti password/ganti role) — fitur baru lengkap 4-layer + fix bug lama `ip_address` selalu null di `audit_logs` (sesi 41, task005)**. **Fix resolusi high margin level-kategori vs level-produk: `fetchHmDetail` (tab Penetrasi Kategori) & `fetchCategoryProducts` (drill-down) sebelumnya samakan "kategori punya ≥1 produk HM" dengan "semua produk kategori itu HM" — sekarang keduanya mirror resolusi yang sama (`hm_cat_level`/`hm_product_level` vs `hm_products` CTE), angka baris kategori & drill-down konsisten. `getCategoryProducts` balikin `meta.summary` (agregat produk terfilter, query terpisah supaya tetap 0 bukan hilang kalau kosong) (task008, 2026-07-26)**. **Fix RBAC lanjutan (task022, 2026-08-06): `resolveDormantMonths()` bocor lintas company saat `company_id='all'` (threshold dormant salah untuk user non-superadmin, ditemukan lewat laporan M5 beda antar user); 3 endpoint Settings (`channel-divisions/unmapped-channels`, `divisions/values`, `item-types/values`) tidak pernah panggil `resolveCompanyScope()` sama sekali — dikonfirmasi exploitable di production (user company A baca data company B lewat `?company_id=`). Semua fix + audit ulang M1–M10 dgn 4 user berbeda, hasil bersih.** |
...
| High Margin      | `/products/high-margin` | 2 tabs: Penetrasi Kategori + Upsell Targets — **real backend**. **Klik baris kategori di tab Penetrasi buka dialog drill-down produk, difilter cuma produk yang ditandai high margin + badge per produk (task007/task008, 2026-07-26)** |
...
## Task007/008 — Drill-down Produk High Margin + Fix Resolusi Level Kategori vs Level Produk (2026-07-26)

**Baca juga:** `docs-v2/task/task007.md`, `docs-v2/task/task008.md`, `docs-v2/features/high-margin-products.md` §9

### task007 — Drill-down awal (ternyata salah scope)
Klik baris kategori di tab "Penetrasi Kategori" buka `CategoryProductsDialog` (dialog yang sama dipakai halaman `/products` & tab Target Upsell) menampilkan daftar produk dalam kategori itu. **Ternyata salah** — dialog itu (`fetchCategoryProducts`) cuma filter `product_category_id`, jadi menampilkan SEMUA produk yang pernah terjual di kategori, bukan cuma yang ditandai high margin.

### task008 — Fix filter drill-down + sinkronisasi tabel utama
Akar masalah: `high_margin_products` bisa ditandai di **level kategori** (`product_category_id` diisi → semua produk di kategori itu ikut HM) atau **level produk** (`product_id` diisi → HANYA produk itu, produk sibling lain di kategori yang sama TIDAK ikut). Sebelumnya kode di beberapa tempat menyamakan "kategori punya ≥1 produk HM" dengan "semua produk kategori itu HM" — salah untuk kasus level-produk.

**Fix** (mirror resolusi yang sama di 2 tempat):
- `category-products.repository.ts` — CTE `hm_products` resolve produk yang efektif HM (union: ditandai langsung ATAU kategorinya ditandai). Param baru `onlyHighMargin`/`only_high_margin` (opsional, default `false` — 2 pemakai lain dialog: tab Target Upsell & halaman `/products` biasa, TIDAK berubah perilaku).
- `high-margin-penetration.repository.ts` (`fetchHmDetail`, baris kategori tabel utama tab Penetrasi) — `hmCatsCte` dipecah jadi `hm_cat_level` (kategori ditandai langsung) + `hm_product_level` (produk spesifik ditandai), filter transaksi jadi `product_category_id IN (hm_cat_level) OR product_id IN (hm_product_level)`. Sebelumnya baris kategori ikut menjumlah transaksi produk sibling yang tidak ditandai HM.
- Kartu summary di drill-down (mode `highMarginOnly`) sekarang pakai `meta.summary` (agregat produk yang sudah difilter, dihitung query terpisah biar tetap balik 0 bukan hilang kalau hasil kosong) — bukan `category.total_revenue` dkk yang dikirim caller (itu angka kategori utuh, beda dari daftar produk terfilter di bawahnya).
- Badge "High Margin" per produk ditambah di tabel drill-down (`is_high_margin` per baris, dari CTE `hm_products` yang sama) — sebelumnya status ini cuma kelihatan di level kategori.

**Diverifikasi:** cross-check query SQL manual (kategori "RECEIPT PRINTER THERMAL KASSEN" — 10 produk, 1 ditandai HM), `bunx tsc --noEmit` bersih, `bun test` backend 73 pass/0 fail.

### Fix tabel "naik-turun tidak sejajar" (dampak ke seluruh aplikasi)
Ditemukan waktu verifikasi visual task008: `.MuiDataGrid-cell` (`ResponsiveListView`, dipakai 23 halaman) defaultnya `display:block` — teks polos numpang baseline garis teks, tapi Chip/badge (`inline-flex`) tidak ikut center otomatis, jadi posisinya beda dgn sel teks di baris yang sama. Fix: cell dipaksa `display:flex; align-items:center`. Kolom yang isinya LIST beberapa chip (bisa wrap 2+ baris, mis. `categories_bought`/`missing_high_margin_categories` di tab Target Upsell) ditandai `cellClassName: 'wrap-chips-cell'` (tetap rata atas — kalau ikut center, konten yang lebih tinggi dari baris meluber ke atas *dan* bawah sekaligus) + `getRowHeight="auto"` di tabel Target Upsell supaya semua kategori rekomendasi kelihatan penuh (sebelumnya kepotong di baris fixed 52px, chip yang tidak muat disembunyikan total).

### Fix icon PWA blank/putih polos
`icon-192.png`, `icon-512.png`, `apple-touch-icon.png` cuma berisi logo semanggi putih TANPA background gelap — invisible di homescreen. `icon-maskable-512.png` desainnya beda sendiri (flower solid tebal, bukan semanggi outline) — brand tidak konsisten antar platform (Android pakai maskable, desktop/iOS pakai icon lain). Ketiga file digenerate ulang pakai desain identik (`favicon.svg`/`AppLogo.tsx` — semanggi outline putih + lingkaran, di atas `#0a0a0f`); maskable diskalakan 0.7x supaya masuk safe zone (~80% diameter tengah), tidak kepotong mask lingkaran/squircle Android.

### Perencanaan (belum diimplementasi): Infra Multi-Entitas
Diskusi rencana ke depan — deploy ke VPS sendiri, `semanggi.com` sebagai root domain, entitas (MKO/KNT/SKI) bisa dapat subdomain/domain sendiri. Didokumentasikan di `docs-v2/task/task009.md` (planning only) — isolasi data antar entitas SUDAH otomatis terjamin oleh `company_id` scoping + RBAC yang ada, domain baru cuma soal "pintu masuk"/branding, opsional ditambah host-check saat login kalau mau pembatasan lebih ketat.
...
| Division per branch — frontend belum expose | Dev | Todo | Backend sudah siap (`divisions.branch_id` nullable, sudah diterima `createDivisionSchema`), frontend `Settings/Divisions` belum ada UI pilih branch. Ditemukan 2026-08-03, task019 session. |
| Channel division tidak inherit branch dari division | Dev | Todo (arsitektur, belum diputuskan) | `channel_divisions.division_id` FK ke `divisions.id` yang punya `branch_id`, tapi `import.repository.ts:150-166` cuma ambil `divisions.key`, `branch_id` dibuang — branch invoice ditentukan mekanisme terpisah (`branch_name` teks mentah). Kalau mau channel mapping ke division branch-specific ikut menentukan branch invoice, perlu: (1) select `divisions.branch_id` di query itu, (2) putuskan prioritas vs resolusi `branch_name` teks yang sudah ada. Ditemukan 2026-08-03, task019 session. |
| Platform (TikTok/Shopee/Tokopedia/dst) belum jadi dimensi filter | Dev | Todo | Saat ini "platform" cuma tersirat di teks `channel_name` (mis. "SHOPEE COS", "TIKTOK CST"), tidak ada kolom eksplisit — kebutuhan cuma untuk FILTER laporan (bukan RBAC/threshold, sudah dikonfirmasi user). Rencana scope kecil: tambah kolom opsional `platform` di `channel_divisions` (bukan dimensi/tabel baru terpisah, tidak ikut ke RBAC seperti division), lalu tambah dropdown filter "Platform" di halaman Transactions/Analisis yang query berdasarkan kolom itu. Ditemukan 2026-08-04, task020 session (kasus import KNT). |
